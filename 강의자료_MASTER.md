...
> **"공식 API가 없어도, 내가 원하는 정보를 AI가 알아서 가져오게 만드는 4단 사다리 — 그리고 그걸 LLM이 대화 중 스스로 호출하게 MCP로 패키징하는 법."**

## 🪜 핵심 프레임 — "정보 수집 4단 사다리" (이 그림 하나로 관통)
...
4순위  브라우저 자동화 직접    ──▶  Playwright · cmux browser     (사람 화면 그대로)
       ...
       ───────────────────────────────────────────────
       ⭐ 위 1~4를 LLM이 대화 중 스스로 호출 → MCP 서버로 패키징 (claude mcp add)
```
...
박준이 실제 Claude Code 환경에서 쓰는 수집 도구를 4단 사다리에 매핑하면:
...
> 1~4단은 "내가 도구를 골라 수집"한다. Gemini는 **검색·읽기·종합을 AI가 알아서** 하게 한다. 난이도 사다리로 시연하면 효과가 극적: ① 그냥 LLM(컷오프 한계) → ② grounding 한 줄(실시간+출처) → ③ Deep Research(자율 보고서). *(상세·인용추출: `_research/R1_gemini_deepresearch.md`)*
...
**하네스 5층**: (A)CLAUDE.md·서브에이전트·command·hooks·settings → (B)Skill → (C)MCP → (D)프롬프트 → (E)검증. *오늘 이 강의 자료 자체가 (A)서브에이전트 병렬 + (E)cmux 1차 검증으로 만들어진 실례다.*
...
### (B) Skill 패키징 — 절차를 파일로
`SKILL.md` frontmatter의 `description`이 **Claude가 언제 쓸지 판단하는 근거**(트리거 키워드를 앞에 풍부히 — 박준 web-to-markdown·korean-law가 모범). 골격 예: `research-ladder` 스킬(주제→4단 사다리→출처포함 마크다운 저장). 전체 SKILL.md + `scripts/collect.py`는 `_research/R3` §B-3에 그대로 복붙 가능.

### (C) MCP 만들기 — 공식 API를 도구로
LLM이 대화 중 스스로 호출. 최소 골격(Python 공식 `mcp` SDK):
```python
...
> **"임의 주제 한 줄 → AI가 4단 사다리로 수집 → 환각 스스로 검증 → frontmatter 위키 페이지 → 지식그래프·팟캐스트 → 슬래시커맨드/스케줄로 매주 자동 갱신"** = 검증된 나만의 백과사전. *(0~7단계 전체 프롬프트·산출물·체크리스트: `_research/R4_llmwiki_capstone.md` — 그대로 따라하면 완성)*

```
주제 한 줄 ─▶ (1)수집계획 → (2)4단 사다리 수집 → (3)환각 교차검증 →
...
| 2 수집 | 항목별 4단 사다리로 수집, 1차 원본 즉시 `sources_raw/`에 보관 | §1~4, `code/` |
...
1. **공식 API 먼저** (arXiv 무료·무키, 법제처 OC키) — 안정·합법·구조화
...
6. **반복할 일은 환경으로** — Claude Code 하네스(CLAUDE.md·서브에이전트·command·hooks) + Skill + MCP + 검증 루프. 코드 내장은 Agent SDK.
7. **검증이 신뢰의 핵심** — 1차출처·다중에이전트 교차검증·verify_citations·외부콘텐츠=데이터(인젝션 방지)
8. **캡스톤** — 이 전부를 엮어 "스스로 자라는 나만의 검증된 LLM Wiki"

> 핵심 한 줄: **도구를 고르는 법(1~5) + 그걸 Claude Code 환경으로 굳히는 법(6~7) + 검증으로 환각을 잡는 법 = 어떤 자료조사든 AI에게 맡기고 나만의 검증된 지식 백과를 쌓는 시스템.**
